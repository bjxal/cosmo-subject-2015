...
function share(type){
    var jiathis_config = {
        ...
        title: "",
        content: $("h1.tit").html()+" HiCOSMO ",
        pic:$(".top_img img").attr("src")
    };
    var tourl = '';
    switch( type ){
        case 'weixin':
    ...
    tourl = tourl.replace('$title',jiathis_config.title).replace('$content',jiathis_config.content).replace('$pic',jiathis_config.pic).replace('$url',jiathis_config.url).replace('$back',jiathis_config.url);
    console.log(tourl)
