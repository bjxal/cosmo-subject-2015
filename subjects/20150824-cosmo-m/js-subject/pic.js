...
//	}
	...
	pad.Gallery = function (options) {
		...
		this._timer =0 ;
		var o = this.o = {
			activeThumbCls: "activeThumbCls",'photoIndex': $('photoIndex'),'photoCount':$('photoCount'),'goBack':$('goBack'),'photoDown':$('photoDown'),'picDiv':$('picDiv'),'photoView': $('photoView'),'photo': $('photo'), 'photoPrevLoading':$('photoPrevLoading'),'photoLoading': $('photoLoading'),'photoDesc': $('photoDesc'),'thumbAll':$('thumbAll'),'wrapper':$('wrapper'),'scroller':$('scroller'),'photoAll':$('photoAll'),'desc':$('desc'),'photoTitle_pad':$('photoTitle_pad'),'prevSet': $('prevSet'), 'nextSet': $('nextSet'),'thumb': $('thumb'),'photoSave':$('photoSave'),'title_pad':$('title_pad'),'picList_b':$('picList_b'),'defaultDesc':$('defaultDesc'),'imgBox':$('imgBox'),'listBottom':$('listBottom')
		}
		for (var i in o) {
		...
		function loaddata(){
			...
			if(t.loadrecommend.length>0){
				...
				//var content = '<h2>�����Ƽ�:</h2><ul id="recommend">';
				var content = '';
		...
		addListener(o.wrapper,'click',function(){
			...
			if(value<=10 && value>=-10){
				if(t.o.title_pad.style.top=="-17px"){
					...
					O.setStyle(t.o.title_pad, "top","-17px");
		...
		addLi:function(){
			...
			while((obj =mb.loadlazyQuene.shift())!=null){
				...
				addListener(li,'touchend',function(){
					...
					if(value<=10 && value>=-10){
						var liArr = mb.m.photoAll.getElementsByTagName("li");
						if(mb.m.menu_mb.style.top=="-44px"){
							mb.Topshow();
						}
						else{
							mb.Tophidden();
