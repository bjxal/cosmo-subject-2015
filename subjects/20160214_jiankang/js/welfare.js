$(document).ready(function(){
    ...
    $(".top,.tit").on("click",function(e){
        var $tar = $(e.target);
        if($tar.attr("class").indexOf("noBg")!=-1) return;f
    ...
    $(".login_btn").on("click",function(e){
        ...
        if(ajax){
            var img_url = $("#result1").html()||"";
            var desc = $(".activity_share #input_5").html();
            img_url = (img_url=="")? $.cookie("picurl"):img_url;
            $.ajax({
                type:"POST",
                url:"http://m.trends-health.com.cn/files/eventapi.php?c=Trendshealthm_Activitypic&a=Addcommentajax",
                data:{"picdescribe":desc,"commentid": $.cookie("commentid"),"commentpic":img_url},
                dataType: "json",
